...
if __name__=='__main__':
    ...
    for data_idx in range(dataset.num_samples):
        # Load data from dataset
        # objects = dataset.get_label_objects(data_idx)
        # objects[0].print_object()
        img = dataset.get_image(data_idx)
        ...
        print(' -------- LiDAR points projected to image plane, processing %06d--------'%(data_idx))
        img_filename = os.path.join('E:/MyFiles/desktop/kitti-object-depth/%06d.png'%(data_idx))
        img1 = np.zeros((img_height,img_width,img_channel), np.uint8)
        img2 = show_lidar_on_image(pc_velo, img1, calib, img_width, img_height) 
        cv2.imwrite(img_filename,img2)
    ...
    # for i in range(imgfov_pts_2d.shape[0]):
    #     m=int(np.round(imgfov_pts_2d[i,0]))-1
    #     n=int(np.round(imgfov_pts_2d[i,1]))-1
    #     img1[n,m,:] = imgfov_pc_rect[i,2]
